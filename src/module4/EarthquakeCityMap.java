...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		HashMap<String, Integer> numsLandQuakes = new HashMap<String, Integer>();
		for (Marker m : quakeMarkers) {
			EarthquakeMarker em = (EarthquakeMarker)m;
			if (em.isOnLand) {
				String country = (String)m.getProperty("country");
				if (numsLandQuakes.containsKey(country)) {
					numsLandQuakes.put(country, numsLandQuakes.get(country) + 1);
				}
				else {
					numsLandQuakes.put(country, 1);
				}
			}
			else {
				numOceanQuakes++;
		...
		numsLandQuakes.forEach((k, v) -> System.out.println(k + " : " + v)); 
